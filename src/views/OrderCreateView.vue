<template>
  <div>
    ...
    <div class="scroll-content has-header has-footer overflow-scroll">
      ...
      <!--</scroller>-->
      <div>
        <div class="font-size-small assertive padding">
          为响应国家包裹实名制要求，请填写<strong class="font-size">真实姓名</strong>，否则可能无法送货
        </div>
        ...
        <group title="商品信息">
          ...
          <cell title="运费">
            <span slot="after-title" class="font-size-small">(包邮)</span>
            {{freight|price}}
          </cell>
          <cell title="税费">
            <span slot="after-title" class="font-size-small">(包税)</span>
            {{tax|price}}
          </cell>
          <cell title="商品总价">
            {{totalPrice|price}}
          </cell>
          <cell :title="selectedCartItems.length|productNumFormat">
            实付：<strong class="assertive" v-text="realPay|price"></strong>
          </cell>
    ...
    <div class="bar bar-footer">
      <h4 class="title">实付：<span class="assertive">{{realPay|price}}</span></h4>
