...
export function syncCartItems(store, uid) {
	ShoppingCart.fetch(uid).then(resp => {
...
export function insertOrUpdate(store, params) {
	var cartItem = find(store.state.cartItems, {
		id: params.productId
	});
	...
	if (cartItem) {
		...
		ShoppingCart.updateById(cartItem.cartid, params.amount).then(() => {
			cartItem.num = params.amount;
		});
	} else {
		ShoppingCart.addById(params.productId, params.uid, 1).then(resp => {
...
export function removeCartItem(store, id) {
	ShoppingCart.removeById(id).then(resp => {
...
export function syncConsignees({
	...
	Consignee.fetch(memberId).then(resp => {
...
export function removeConsigneeById({
	...
	Consignee.removeById(id).then(resp => {
...
export function updateConsignee({
	...
	consignee.update(receid, memberId).then(resp => {
